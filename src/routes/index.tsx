    ...
    <PublicShell>
      <div className="space-y-3 sm:space-y-4">
        <div className="rounded-2xl sm:rounded-3xl bg-gradient-to-br from-primary via-primary to-primary-glow p-4 text-primary-foreground shadow-card sm:p-5">
          <div className="text-[10px] font-medium uppercase tracking-wide opacity-80 sm:text-xs">
            {t("fundBalance")}
          </div>
          <div className="mt-1 text-2xl font-extrabold tracking-tight sm:text-3xl">
            {formatRs(sum.balance)}
          </div>
        ...
        <div className="grid grid-cols-2 gap-2 sm:gap-3">
          <StatCard
            ...
            value={sum.totalFamilies}
            icon={Users}
            tone="success"
            hint={`${sum.activeFamilies} ${t("active").toLowerCase()}`}
          />
          <StatCard
            label={t("welfareEvents")}
            ...
            <ul className="divide-y divide-border/60">
              {recentColl.map((c) => (
                <li key={c.id} className="flex items-center justify-between py-2 sm:py-2.5">
                  ...
                  <div className="shrink-0 text-xs font-semibold text-success sm:text-sm">
                    +{formatRs(c.amount)}
                  </div>
                </li>
              ))}
        ...
        <SectionCard
          ...
          action={
            <Link to="/expenses" className="text-xs font-medium text-primary">
              {t("viewAll")}
            </Link>
          }
        >
            ...
            <ul className="space-y-2">
              {recentEv.map((e) => (
                <li key={e.id}>
                  <Link
                    to={`/events/${e.id}`}
                    className="flex items-center justify-between rounded-xl border border-border/60 p-3 hover:bg-muted/50"
                  >
                    <div className="min-w-0">
