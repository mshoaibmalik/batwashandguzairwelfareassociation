...
export default function AdminCollections() {
  ...
  const list = useMemo(
    () => [...data.collections].sort((a, b) => b.date.localeCompare(a.date)),
    [data.collections],
  );
      ...
      <div className="mb-3 flex justify-end">
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                {editing ? t("edit") : t("add")} {t("collections")}
              </DialogTitle>
            </DialogHeader>
            <div className="grid gap-3">
              ...
              <Field label={t("type")}>
                <Select
                  value={form.type}
                  onValueChange={(v) => setForm({ ...form, type: v as CollectionType })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                ...
                <>
                  ...
                  <Field label="Months covered">
                    <Select
                      ...
                      onValueChange={(v) => setForm({ ...form, monthsCount: Number(v) })}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        {[1, 2, 3, 4, 5, 6, 12].map((n) => (
                          <SelectItem key={n} value={String(n)}>
                            {n} month{n > 1 ? "s" : ""} · {formatRs(MONTHLY_RATE * n)}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <div className="mt-0.5 flex items-center gap-1.5 text-[11px] text-muted-foreground">
                    <Badge
                      variant={c.type === "monthly" ? "default" : "secondary"}
