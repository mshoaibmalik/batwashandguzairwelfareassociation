...
export default function AdminDashboard() {
  ...
  const catData = Object.entries(byCat).map(([name, value]) => ({
    name: t(name as "food"),
    value,
  }));
  ...
  const recent = [
    ...data.collections.map((c) => ({
      id: c.id,
      type: "in" as const,
      label: data.families.find((f) => f.id === c.familyId)?.name ?? "—",
      amount: c.amount,
      date: c.date,
    })),
    ...data.expenses.map((e) => ({
          ...
          <ul className="divide-y divide-border/60">
            {recent.map((r) => (
              <li key={r.id} className="flex items-center justify-between py-2 sm:py-2.5">
                ...
                <div
                  className={`text-xs sm:text-sm font-semibold ${r.type === "in" ? "text-success" : "text-destructive"}`}
                >
                  {r.type === "in" ? "+" : "−"}
                  {formatRs(r.amount)}
                </div>
