...
export default function EventDetail() {
  ...
  return (
    <PublicShell title={family?.name ?? "Event"}>
      <Link to="/events" className="mb-2 sm:mb-3 inline-flex items-center gap-1 text-xs text-muted-foreground"><ArrowLeft className="h-3 w-3 sm:h-4 sm:w-4" />Back</Link>
      <div className="card-soft p-3 sm:p-4">
        ...
        <div className="text-[11px] text-muted-foreground sm:text-xs">{family?.head} · {formatDate(ev.eventDate)}</div>
              ...
              {expenses.map((e) => (
                <li key={e.id} className="flex items-center justify-between py-2 sm:py-2.5">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[10px] text-muted-foreground sm:text-[11px]">{e.description ?? formatDate(e.date)}</div>
                  </div>
              ...
              {specialColl.map((c) => (
                <li key={c.id} className="flex items-center justify-between py-2 sm:py-2.5">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[10px] text-muted-foreground sm:text-[11px]">{formatDate(c.date)}</div>
                  </div>
                  <div className="text-xs sm:text-sm font-semibold text-success">+{formatRs(c.amount)}</div>
                </li>
              ))}
            </ul>
          )}
